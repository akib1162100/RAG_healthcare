...
function makeActionManager(env) {
    ...
    //  MULTI TAB START
    function updateShTabDetails(tabId, ActionId, controller) {
    ...
    async function _updateUI(controller, options = {}) {
        ...
        controller.config.setDisplayName = (displayName) => {
            ...
            if (controller.action.id){
                ...
                /* if($(menu_xmlid).length){
                    updateShTabDetails(tabID, UpdateAction, controller);
                } */
        ...
        if(enable_multi_tab){
        // MULTI TAB START
        sessionStorage.setItem("sh_current_action", action._originalAction);
        sessionStorage.setItem("sh_current_action_id", action.id);
        // MULTI TAB END
        }
