...
class Provider(models.Model):
    ...
    def graph_api_direct_send_mpm_template(self, template, language, namespace, partner, params):
        if self.graph_api_authenticated:
            ...
            header_text = False
            header = {}
            wa_template_id = self.env['wa.template'].search([('name', '=', template), ('template_type', '=', 'interactive')], limit=1) or self.env.context.get('wa_template')
    ...
    def get_channel_whatsapp(self, partner, user):
        ...
        if not partner or not user:
            return channel
        provider_channel_id = partner.channel_provider_line_ids.filtered(lambda s: s.provider_id.id == self.id) or self.env['channel.provider.line'].sudo().search([('provider_id', '=', self.id)], limit=1)
