...
class WAComposer(models.TransientModel):
    ...
    def default_get(self, fields):
        ...
        self.template_id = self.env['wa.template'].sudo().browse(self.env.context.get('default_template_id')).id if self.env.context.get('default_template_id') else False
        return result
    ...
    attachment_ids = fields.Many2many('ir.attachment', 'wa_compose_message_ir_attachments_rel','wa_wizard_id', 'attachment_id', 'Attachments')
            ...
            if self.template_id:
                context_vals.update(
                    {'template_send': True, 'wa_template': self.template_id, 'active_model_id': active_id,
                     ...
                     'attachment_ids': self.attachment_ids, 'user_id': self.env.ref('base.user_admin')})
